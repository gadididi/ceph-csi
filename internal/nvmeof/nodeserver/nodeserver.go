...
type NvmeConnectionInfo struct {
	...
	ListenerAddress string `json:"listenerAddress"` // TODO - convert to list with listener port.
	ListenerPort    string `json:"listenerPort"`    // TODO - convert to list with listener port.
...
const (
	nvmeofSubsystemNQN    = "nvmeof.SubsystemNQN"
	nvmeofNamespaceID     = "nvmeof.NamespaceID"
	nvmeofNamespaceUUID   = "nvmeof.NamespaceUUID"
	nvmeofListenerAddress = "nvmeof.ListenerIpAddress"
	nvmeofListenerPort    = "nvmeof.ListenerPort"
	nvmeofHostNQN         = "nvmeof.HostNQN"
	defaultTransport      = "tcp"
)

// file name in which image metadata is stashed.
const stashFileName = "nvmeof-metadata.json"
...
func (ns *NodeServer) NodeStageVolume(
	...
	connectionInfo, err := ns.parseVolumeContext(volumeContext)
	if err != nil {
	...
	// Stash volume metadata prior to connecting (useful during Unstage)
	err = ns.stashNvmeVolumeMetadata(connectionInfo, stagingParentPath)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	...
	defer func() {
		if err != nil {
			ns.undoStagingTransaction(ctx, req, connectionInfo, txn)
		}
	}()
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
...
func (ns *NodeServer) NodeUnstageVolume(
	...
	connectionInfo, err := lookupNvmeVolumeMetadataStash(stagingParentPath)
	if err != nil {
		log.UsefulLog(ctx, "failed to find connection metadata: %v", err)

		// If it's mounted but no metadata, that's bad - we don't know how to disconnect
		if isMnt {
			return nil, status.Error(codes.Internal, err.Error())
		}

		// If not mounted and no metadata, volume is already cleaned up
		if os.IsNotExist(err) {
			// Already unstaged - this is fine (idempotent)
			return &csi.NodeUnstageVolumeResponse{}, nil
		}

		// Any other error (permissions, corruption, etc.) is a real error
		return nil, status.Error(codes.Internal, err.Error())
	}

	// Unmapping nvmeof device
	err = ns.disconnectFromSubsystem(ctx, connectionInfo)
	if err != nil {
		log.ErrorLog(ctx, "failed to disconnect from NVMe subsystem: %v", err)

		return nil, status.Error(codes.Internal, err.Error())
	}

	if err = ns.cleanupVolumeMetadata(stagingParentPath); err != nil {
...
func (ns *NodeServer) undoStagingTransaction(
	...
	if transaction.devicePath != "" {
		...
		if err != nil {
			log.ErrorLog(ctx, "failed to disconnect from NVMe subsystem: %v", err)
			// continue on failure to delete the stash file
		}
	}

	// Cleanup the stashed image metadata
	if err = ns.cleanupVolumeMetadata(req.GetStagingTargetPath()); err != nil {
		log.ErrorLog(ctx, "failed to cleanup image metadata stash (%v)", err)
	}
}
...
// parseVolumeContext extracts connection information from volume context.
func (ns *NodeServer) parseVolumeContext(volumeContext map[string]string) (*NvmeConnectionInfo, error) {
	...
	listenerAddress, ok := volumeContext[nvmeofListenerAddress]
	if !ok || listenerAddress == "" {
		return nil, errors.New("missing listener address in volume context")
	}

	listenerPort, ok := volumeContext[nvmeofListenerPort]
	if !ok || listenerPort == "" {
		return nil, errors.New("missing listener port in volume context")
	}

	hostNQN, ok := volumeContext[nvmeofHostNQN]
	if !ok || hostNQN == "" {
...
func (ns *NodeServer) stashNvmeVolumeMetadata(connectionInfo *NvmeConnectionInfo, stagingParentPath string) error {
	metadataFile := filepath.Join(stagingParentPath, stashFileName)
	encodedBytes, err := json.Marshal(connectionInfo)
	if err != nil {
		return fmt.Errorf("failed to marshal JSON metadata: %w", err)
	}

	err = os.WriteFile(metadataFile, encodedBytes, 0o600)
	if err != nil {
		return fmt.Errorf("failed to stash JSON metadata at path (%s): %w", metadataFile, err)
	}

	return nil
}

// lookupNvmeVolumeMetadataStash reads and returns stashed NVMe connection metadata at passed in path.
func lookupNvmeVolumeMetadataStash(metaDataPath string) (*NvmeConnectionInfo, error) {
	var connInfo NvmeConnectionInfo

	fPath := filepath.Join(metaDataPath, stashFileName)
	encodedBytes, err := os.ReadFile(fPath) // #nosec - intended reading from fPath
	if err != nil {
		if !os.IsNotExist(err) {
			return nil, fmt.Errorf("failed to read stashed JSON connection metadata from path (%s): %w", fPath, err)
		}

		// File doesn't exist - this is expected for first-time mounts
		return nil, fmt.Errorf("metadata stash file not found at %s: %w", fPath, err)
	}

	err = json.Unmarshal(encodedBytes, &connInfo)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal stashed JSON connection metadata from path (%s): %w", fPath, err)
	}
...
func (ns *NodeServer) connectToSubsystem(ctx context.Context, info *NvmeConnectionInfo) (string, error) {
	...
	connectReq := &nvmeof.ConnectRequest{
		...
		Addresses:    []string{info.ListenerAddress},
		Ports:        []string{info.ListenerPort},
	...
	if err != nil {
		// Attempt cleanup
		log.ErrorLog(ctx, "failed to get namespace uuid %s: %w", info.NamespaceUUID, err)

		return "", ns.disconnectFromSubsystem(ctx, info)
	}

	return devicePath, nil
}

// disconnectFromSubsystem disconnects from the NVMe-oF subsystem.
func (ns *NodeServer) disconnectFromSubsystem(ctx context.Context, connectionInfo *NvmeConnectionInfo) error {
	// The initiator handles reference counting internally
	return ns.initiator.DisconnectSubsystem(ctx, connectionInfo.SubsystemNQN)
}

// cleanupVolumeMetadata removes volume metadata.
func (ns *NodeServer) cleanupVolumeMetadata(stagingTargetPath string) error {
	metadataFile := filepath.Join(stagingTargetPath, stashFileName)
	if err := os.Remove(metadataFile); err != nil {
		return fmt.Errorf("failed to cleanup stashed JSON data (%s): %w", metadataFile, err)
	}

	return nil
}

// createStageMountPoint.
...
func (ns *NodeServer) mountVolumeToStagePath(
	...
	mountOptions := []string{"_netdev"} // TODO : do i need it??
